...
public final class MerConfigCache implements ICache {
	
	private static final Logger logger = Logger.getLogger(MerConfigCache.class);
	
	private static MerConfigCache instance = new MerConfigCache();
	
	private Properties props;
	
	private MerConfigCache() { }
	
	public static MerConfigCache getInstance() {
		return instance;
	}
	...
	public static String getConfig(String configKey, String defaultVal){
		try {
			String val =  instance.props.getProperty(configKey);
	...
	private void load() {
		...
		if (!StringUtil.isBlank(MerConstants.CONFIG_PATH)) {
			String configFileFullPath = MerConstants.CONFIG_PATH + MerConstants.PORTAL_CONFIG_FILE_NM;
			...
			propsTemp = PropertyUtil.getProperties(MerConstants.PORTAL_CONFIG_FILE_NM, this);
		}
		props=propsTemp;
