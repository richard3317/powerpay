...
@Component("HaikeOfflineAccChkTask")
public class HaikeOfflineAccChkTask extends ChnlAccChkBaseTask {
		
	
	@Override
	protected void doTask() {
		delteTotalCheckResult(CHANNEL._05);
		delteCheckResult(CHANNEL._05);
		...
		List<ChnlVirMchntSettleInfo> chnlFiles = queryChnlSettleInfo(CHANNEL._05);
		...
		for(ChnlVirMchntSettleInfo chnlFile:chnlFiles){
			...
			if(!f.exists()){
				throw new RuntimeException("海科线下对账文件不存在["+f+"]");
			}else{
				...
				if(errContent.size()>1){
					reFundFilePath = storeToRefundFile(f.getName(),errContent,CHANNEL._05);
					checkRes = "0";
				}
				insertTotalCheckResult(f.getName(),checkRes,reFundFilePath,CHANNEL._05);
			}
		}
		if(!allTranLogs.isEmpty()){
			List<Map<String,String>> temp = new ArrayList<Map<String,String>>();
			temp.addAll(allTranLogs.values());
			batchSavePendingResult(temp,CHANNEL._05);
		}
		
		if(!checkLogs.isEmpty()){
			batchSaveCheckedResult(checkLogs,CHANNEL._05);
		}
	...
	private Map<String,Map<String,String>> loadTransLog(){
		List<Map<String,String>> allTransLogs =queryAllTransLogs(CHANNEL._05);
		...
		allTransLogs.clear();
		logger.info(batchDt+"日海科线下成功交易["+res.size()+"]");
		return res;
	}
	
	private Map<String,Map<String,String>> loadPendingTransLog(){
		List<Map<String,String>> pendingTransLogs =queryPendingTransLogs(CHANNEL._05);
