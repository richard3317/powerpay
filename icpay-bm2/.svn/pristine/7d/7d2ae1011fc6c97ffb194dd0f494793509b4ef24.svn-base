...
<html>
	...
	<meta name="referrer" content="no-referrer" />
		<@head label="电汇自充" base="offpay" qryFuncCode="6001010000"
		detailFlg=true detailFuncCode="6001010002" detailUrl='"detail.do?transSeqId="+sel.transSeqId' detailDivWidth=800 detailDivHeight=550 >
			var t = null;
			var mFlg = false;
			var dealing = false;
			var scd = 0;
			...
			$("#apprBtn").click(function() {
				...
				if (sel == null) {
					...
					$.jumpTo("${ctx}/offpay/offpayAppr.do?transSeqId=" + sel.transSeqId + "&mon=" + sel.extTransDt + "&orderState=" + sel.txnState);
				}
			});
			...
			$("#exportTransLog").click(function(){
				...
				var url ="${ctx}/offpay/exportTransLog.do?"+qryStr;
				<#--  console.log(url); -->			
				$.jumpTo(url);
			});
			...
			$("#viewImage").click(function() {
				$(this).once( function(){
					...
					if (sel == null) {
						...
						var url = "${ctx}/offpay/viewImage.do?transSeqId=" + sel.transSeqId;
						crtViewDialog("detailDiv", "查看汇款凭证", url, 800, 550);
					}
			...
			function playSound(){
				var audioElement = document.createElement('audio');
			  	audioElement.setAttribute('src', '${ctx}/resources/audio/push_bell.mp3');
			  	audioElement.play();
			}
			
			function countOffPay(){
				$.ajax({
					type : "POST",
					url : baseUrl+"countOffPay.do",
					async: true,
					cache:false,
					data: $("#qryForm").serialize(),
					success : function(mm) {
						var obj = parseJson(mm);
						var offPayOld = $("#countOffPay").val();
						var offPayNew = obj.countOffPay;
						if (offPayNew > offPayOld) {
							playSound();
						}
						$("#countOffPay").val(offPayNew);
					},
					error:function(){
				    	alert("系统异常，请联系管理员！");
				    	return  false;
	...
	<body style="padding: 5px;">
		<@qryDiv qryUrl="/offpay/qry.do" qryFuncCode="6001010001" showBtn=false >
			<table class="qry_tbl">
				<input type="hidden" value='0191' id="transType" name="_QRY_transType"/>
				<input type="hidden" value="0" id="countOffPay"/>
				<tr>
					<td>交易日期：</td>
					<td colspan="3">
		...
		<@qryResultDiv detailFlg=true detailFuncCode="6001010001">
			<@authCheck funcCode='6001010005'>
				<a id="apprBtn" href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-edit'">
				审批</a>
			</@authCheck>
			
			<@authCheck funcCode='6001010001'>
			...
			<@authCheck funcCode='6001010001'>
				<a id="viewImage" href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-man'">查看汇款凭证</a>
			</@authCheck>
			<br/>
