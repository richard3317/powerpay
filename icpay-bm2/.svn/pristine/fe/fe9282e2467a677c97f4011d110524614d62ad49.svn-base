...
@Controller
@RequestMapping("/dailyProfitResultChnlTrans")
public class DailyProfitResultChnlTransController extends BaseController {

	private static final String RESULT_BASE_URI = "dailyProfitResultChnlTrans";
	private static final IEntityTransfer ENTITY_TRANSFER = new IEntityTransfer() {
		...
		public void transfer(Map<String, String> m) {
			...
			String chnlTransTxnAmtSum = m.get("chnlTransTxnAmtSum");
			m.put("chnlTransTxnAmtSum", StringUtil.formateAmt(chnlTransTxnAmtSum));
			String chnlTransAgentProfit = m.get("chnlTransAgentProfit");
			m.put("chnlTransAgentProfit", StringUtil.formateAmt(chnlTransAgentProfit));
			String chnlTransCtTxnAmtSum = m.get("chnlTransCtTxnAmtSum");
			m.put("chnlTransCtTxnAmtSum", StringUtil.formateAmt(chnlTransCtTxnAmtSum));
			String chnlTxnAmtSum = m.get("chnlTxnAmtSum");
			m.put("chnlTxnAmtSum", StringUtil.formateAmt(chnlTxnAmtSum));
			String chnlAgentProfit = m.get("chnlAgentProfit");
			m.put("chnlAgentProfit", StringUtil.formateAmt(chnlAgentProfit));
			String chnlCtTxnAmtSum = m.get("chnlCtTxnAmtSum");
			m.put("chnlCtTxnAmtSum", StringUtil.formateAmt(chnlCtTxnAmtSum));
		}
	};
	...
	public @ResponseBody AjaxResult qry(int pageNum, int pageSize) {
		...
		Pager<DailyProfitResultChnlTrans> p = null;
		if ("lelipay".equals(payName) || "xihapay".equals(payName)) {
			getQryParamMap().put("agentCd", agentCd);
			p = service.selectChnlTransByPage(pageNum, pageSize, this.getQryParamMap());
		} else {
			getQryParamMap().put("agentCd", agentCd);
			p = service.selectChnlTransByPage(pageNum, pageSize, this.getQryParamMap());
		}
		return commonBO.buildSuccResp(BMConstants.PAGER_RESULT, 
				commonBO.transferPager(p, BMConstants.PAGE_CONF_DAILYPROFITRESULTCHNLTRANS, ENTITY_TRANSFER));
	}
	...
	public void exportRpt(Model model, HttpServletResponse response) {
		IDailyProfitResultViewService service = this.getDBService(IDailyProfitResultViewService.class);
		String payName = BMConfigCache.getConfig(BMConstants.CONFIG_KEY_PAY_NAME);
		List<DailyProfitResultChnlTrans> respList = service.selectChnlTransRpt(this.getQryParamMap());
		String qryStartDate = getQryParamMap().get("qryStartDate");
		String qryEndDate = getQryParamMap().get("qryEndDate");
		String fileNm = BMConfigCache.getConfig(BMConstants.CONFIG_KEY_CHNLTRANS_REPORT_FILE_NM) + "-" + DateUtil.now8() + ".xls";
