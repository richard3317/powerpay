...
<html>
	<head>
		<@head label="渠道商户账户流水" base="chnlAccount" qryFuncCode="2000040005">
			$("#backToMngBtn").click(function() {
				$.jumpTo(_ctx + "/chnlAccount/backToManage.do");
			});
			<@authCheck funcCode='2000040006'>
			...
			<@authCheck funcCode='2000040004'>
				...
				function exportData(){
					...
					var _note  = $("#note").val();
					$.jumpTo("${ctx}/chnlAccount/export.do?"
						+"chnlId=${chnlId}"
						+"&mchntCd=${mchntCd}"
	...
	<body style="padding: 5px;">
		<@qryDiv qryUrl="/chnlAccount/accountFlowQry.do" qryFuncCode="2000040005">
			...
			<table class="qry_tbl">
				...
				<tr>
					<td>交易流水号：</td>
					<td>
						<input type="text" name="transSeqId" maxLength="64" />
					</td>
					<td>操作类型：</td>
					<td>
