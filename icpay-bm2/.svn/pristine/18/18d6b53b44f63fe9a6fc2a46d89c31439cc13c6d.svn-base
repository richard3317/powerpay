...
<html>
	<head>
		<@head label="提现审核" base="withdrawAudit" qryFuncCode="6001040000"
		detailFlg=true detailFuncCode="6001040002" detailUrl='"detail.do?transSeqId="+sel.transSeqId' detailDivWidth=800 detailDivHeight=550 >
			var t = null;
			var mFlg = false;
			var dealing = false;
			var scd = 0;
			...
			$("#apprBtn").click(function() {
				...
				if (sel == null) {
					...
					$.jumpTo("${ctx}/withdrawAudit/appr.do?transSeqId=" + sel.transSeqId + "&mon=" + sel.extTransDt + "&orderState=" + sel.txnState);
				}
			});
			...
			$("#exportTransLog").click(function(){
				...
				var url ="${ctx}/withdrawAudit/exportTransLog.do?"+qryStr;
				<#--  console.log(url); -->			
				$.jumpTo(url);
			});
	...
	<body style="padding: 5px;">
		<@qryDiv qryUrl="/withdrawAudit/qry.do" qryFuncCode="6001040001" showBtn=false >
			<table class="qry_tbl">
				<input type="hidden" value='5220' id="transType" name="_QRY_transType"/>
				<tr>
					<td>交易日期：</td>
					<td colspan="3">
		...
		<@qryResultDiv detailFlg=true detailFuncCode="6001040001">
			<@authCheck funcCode='6001040005'>
				<a id="apprBtn" href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-edit'">
				审批</a>
			</@authCheck>
			
			<@authCheck funcCode='6001040001'>
