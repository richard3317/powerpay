...
@Controller
@RequestMapping("/dailyProfitResultEmployee")
public class DailyProfitResultEmployeeController extends BaseController {

	private static final String RESULT_BASE_URI = "dailyProfitResultEmployee";
	...
	public @ResponseBody AjaxResult qry(int pageNum, int pageSize) {
		...
		Pager<DailyProfitResultEmployee> p = null;
		if ("lelipay".equals(payName) || "xihapay".equals(payName)) {
			getQryParamMap().put("agentCd", agentCd);
			p = service.selectEmployeeLpayVpayByPage(pageNum, pageSize, this.getQryParamMap());
		} else {
			getQryParamMap().put("agentCd", agentCd);
			p = service.selectEmployeePpayFpayByPage(pageNum, pageSize, this.getQryParamMap());
		}
		return commonBO.buildSuccResp(BMConstants.PAGER_RESULT, 
				commonBO.transferPager(p, BMConstants.PAGE_CONF_DAILYPROFITRESULTEMPLOYEE, ENTITY_TRANSFER));
	}
	...
	public void exportRpt(Model model, HttpServletResponse response) {
		IDailyProfitResultViewService service = this.getDBService(IDailyProfitResultViewService.class);
		String payName = BMConfigCache.getConfig(BMConstants.CONFIG_KEY_PAY_NAME);
		List<DailyProfitResultEmployee> respList = null;
		if ("lelipay".equals(payName) || "xihapay".equals(payName)) {
			respList = service.selectEmployeeLpayVpayRpt(this.getQryParamMap());
		} else {
			respList = service.selectEmployeePpayFpayRpt(this.getQryParamMap());
		}
		String qryStartDate = getQryParamMap().get("qryStartDate");
		String qryEndDate = getQryParamMap().get("qryEndDate");
		String fileNm = BMConfigCache.getConfig(BMConstants.CONFIG_KEY_EMPLOYEE_REPORT_FILE_NM) + "-" + DateUtil.now8() + ".xls";
