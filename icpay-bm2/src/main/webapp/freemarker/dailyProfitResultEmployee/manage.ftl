...
<html>
	<head>
		<@head label="财报统计" base="dailyProfitResultEmployee" qryFuncCode="9200040001">
			<@authCheck funcCode="9200040002">
				$("#exportBtn").click(function() {
					var qryStr=$("#qryForm").serialize();
					var url =_ctx + "/dailyProfitResultEmployee/exportRpt.do?"+qryStr;
					console.log(url);
					$.jumpTo(url);
				});
			</@authCheck>
	...
	<body style="padding: 5px;">
		<div id="mngDiv" class="easyui-panel" title="各类支付交易环比报表" style="padding: 10px;">
			<@qryDiv qryUrl="/dailyProfitResultEmployee/qry.do" qryFuncCode="9200040001">
				<table class="qry_tbl">
					<tr>
						<td>交易类型</td>
						<td>
