...
public class WithdrawAuditController extends TransLogBaseController {

	private static final String RESULT_BASE_URI = "withdrawAudit";
	...
	public @ResponseBody AjaxResult qry(Model model, int pageNum, int pageSize, String mon) {
		...
		return this.query(model, BMConstants.PAGE_WITHDRAW_AUDIT, this.getQryParamMap(), pageNum, pageSize, mon);
	}

	@RequestMapping(value = "/detail.do")
	public String detail(Model model, String transSeqId, String mon) {
		return super.detail(model, RESULT_BASE_URI, BMConstants.PAGE_WITHDRAW_AUDIT, transSeqId, mon);
	}
	...
	public String approve(Model model, String transSeqId, String mon, String txnState) {
		...
		model.addAttribute(BMConstants.ENTITY_RESULT, commonBO.transferEntity(entity, BMConstants.PAGE_WITHDRAW_AUDIT, ENTITY_TRANSFER));
	...
	public @ResponseBody AjaxResult adjustTxnStateSubmit(Model model, TxnLogView tr, String mon, String apprResult,
		...
		try {
			...
			info("[InternalGatewayClient][提现审核] "+notifyReq  + " 操作员：" + super.getSessionUsrId());
			resp = InternalGatewayClient.notify(notifyReq);
			info("[InternalGatewayClient][提现审核通知响应] "+resp);
			return commonBO.buildAjaxResp(AjaxRespEnums.SUCC,"审核请求成功");
		}catch(Exception e ) {
			error("[InternalGatewayClient][提现审核结果通知][ERROR] 调用内部取现服务接口异常：" + e.getMessage(), e);
	...
	public void exportTransLog(Model model, String mon, HttpServletResponse response) {
		...
		super.export(model, BMConstants.PAGE_WITHDRAW_AUDIT, this.getQryParamMap(), mon, filename, response);
	}
