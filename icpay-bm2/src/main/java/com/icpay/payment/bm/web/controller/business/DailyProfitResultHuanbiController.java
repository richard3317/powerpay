...
@Controller
@RequestMapping("/dailyProfitResultHuanbi")
public class DailyProfitResultHuanbiController extends BaseController {

	private static final String RESULT_BASE_URI = "dailyProfitResultHuanbi";
	...
	public @ResponseBody AjaxResult qry(int pageNum, int pageSize) {
		...
		Pager<DailyProfitResultHuanbi> p = null;
		if ("lelipay".equals(payName) || "xihapay".equals(payName)) {
			getQryParamMap().put("agentCd", agentCd);
			p = service.selectHuanbiLpayVpayByPage(pageNum, pageSize, this.getQryParamMap());
		} else {
			getQryParamMap().put("agentCd", agentCd);
			p = service.selectHuanbiPpayFpayByPage(pageNum, pageSize, this.getQryParamMap());
		}
		return commonBO.buildSuccResp(BMConstants.PAGER_RESULT, 
				commonBO.transferPager(p, BMConstants.PAGE_CONF_DAILYPROFITRESULTHUANBI, ENTITY_TRANSFER));
	}
	...
	public void exportRpt(Model model, HttpServletResponse response) {
		IDailyProfitResultViewService service = this.getDBService(IDailyProfitResultViewService.class);
		String payName = BMConfigCache.getConfig(BMConstants.CONFIG_KEY_PAY_NAME);
		List<DailyProfitResultHuanbi> respList = null;
		if ("lelipay".equals(payName) || "xihapay".equals(payName)) {
			respList = service.selectHuanbiLpayVpayRpt(this.getQryParamMap());
		} else {
			respList = service.selectHuanbiPpayFpayRpt(this.getQryParamMap());
		}
		String qryStartDate = getQryParamMap().get("qryStartDate");
		String qryEndDate = getQryParamMap().get("qryEndDate");
		String fileNm = BMConfigCache.getConfig(BMConstants.CONFIG_KEY_HUANBI_REPORT_FILE_NM) + "-" + DateUtil.now8() + ".xls";
