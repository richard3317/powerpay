...
//			m.put("settlePeriodDesc",EnumUtil.translate(SettleEnums.SettlePeriod.class, settlePeriod, false));
	...
	public @ResponseBody AjaxResult addSubmit(Model model, ChnlMchntInfo chnlMchntInfo) {
		...
		RiskEvent.bm()
			...
			.message(String.format("用户： %s, 新增渠道商户", commonBO.getSessionUser().getUsrId()))
...
//		
	...
	public @ResponseBody AjaxResult delete(ChnlMchntInfo key) {
		...
		RiskEvent.bm()
			...
			.message(String.format("用户： %s, 删除渠道商户", commonBO.getSessionUser().getUsrId()))
	...
	public @ResponseBody AjaxResult editSubmit(Model model, ChnlMchntInfo chm) {
		...
		RiskEvent.bm()
			...
			.message(String.format("用户： %s, 修改渠道商户", commonBO.getSessionUser().getUsrId()))
...
//				this.judegIsNullMul100(cbm.getTxAmtMax(), "0"), txDayMax, txCardDayMax, txT0Percent, txT0Percent, txT0Percent, txT0Percent, null, null);
		...
		RiskEvent.bm()
			...
			.message(String.format("用户： %s, 渠道计费方式新增。渠道：%s, 渠道商户号： %s, 交易类型： %s, 币别： %s", commonBO.getSessionUser().getUsrId(), cbm.getChnlId(), cbm.getChnlMchntCd(), cbm.getIntTransCd(), EnumUtil.translate(BmEnums.CurrCdType.class, cbm.getCurrCd(), false)))
			.params(eventParams)
			.params(extraParams)
			.submit();
	...
	public @ResponseBody AjaxResult transParamDelete(ChnlMchntInfoSubKey key) {
		...
		RiskEvent.bm()
			...
			.message(String.format("用户： %s, 渠道计费方式删除。渠道：%s, 渠道商户号： %s, 交易类型： %s, 币别： %s", commonBO.getSessionUser().getUsrId(), key.getChnlId(), key.getChnlMchntCd(), key.getIntTransCd(), EnumUtil.translate(BmEnums.CurrCdType.class, key.getCurrCd(), false)))
			.params(eventParams)
			.params(extraParams)
			.submit();
	...
	public @ResponseBody AjaxResult transParamUpdSubmit(Model model, ChnlMchntInfoSubMapping chm) {
		...
		RiskEvent.bm()
			...
			.message(String.format("用户： %s, 渠道计费方式修改。渠道：%s, 渠道商户号： %s, 交易类型： %s, 币别： %s", commonBO.getSessionUser().getUsrId(), chm.getChnlId(), chm.getChnlMchntCd(), chm.getIntTransCd(), EnumUtil.translate(BmEnums.CurrCdType.class, chm.getCurrCd(), false)))
			.params(eventParams)
			.params(extraParams)
			.submit();
