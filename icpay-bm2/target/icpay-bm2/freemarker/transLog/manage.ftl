...
<html>
	<head>
		<@head label="${label}" base="transLog" qryFuncCode="5000010001"
			detailFlg=true detailFuncCode="5000010002" detailUrl='"detail.do?transSeqId=" + sel.transSeqId + "&mon=" + $("#monSel").val()' detailDivWidth=800 detailDivHeight=550 >
			$("#monSel").monthSel();
			...
			$("#exportTransLog").click(function(){
				...
				var url ="${ctx}/transLog/exportTransLog.do?"+qryStr;
				<#--  console.log(url); -->			
				$.jumpTo(url);
			});
			...
			$("#transSateModify").click(function() {
				...
				if (sel == null) {
					...
					$.jumpTo("${ctx}/transLog/transSateModify.do?transSeqId=" + sel.transSeqId + "&mon=" + sel.extTransDt + "&orderState=" + sel.txnState);
				}
			});
			...
			$("#mandatoryModify").click(function() {
				...
				if (sel == null) {
					...
					$.jumpTo("${ctx}/transLog/mandatoryModify.do?transSeqId=" + sel.transSeqId + "&mon=" + sel.extTransDt + "&orderState=" + sel.txnState);
				}
			});
			...
			<#-- $("#differentModify").click(function() {
				...
				if (sel == null) {
					...
					$.jumpTo("${ctx}/transLog/differentModify.do?transSeqId=" + sel.transSeqId + "&mon=" + sel.extTransDt + "&orderState=" + sel.txnState);
				}
			});-->
				...
				$(this).once( function(){
					...
					if (sel == null) {
						...
						if(now.getTime() > creaTime.getTime()){
							...
							if (!r) return false;
							var url = "${ctx}/transLog/queryChnlResult.do?transSeqId=" + sel.transSeqId + "&orderDate=" + sel.extTransDt;
	...
	<body style="padding: 5px;">
		<@qryDiv qryUrl="/transLog/qry.do" qryFuncCode="5000010001">
			<table class="qry_tbl">
				<tr>
					<td>全文检索：</td>
					<td>
